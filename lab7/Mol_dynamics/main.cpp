...
int main(int argc, char *argv[]){ 
  ...
  for(int istep=0; istep < N_blocks; ++istep){
    ...
    for(int j=0; j<L;j++){
      ...
      Mdin.Accumulate(); //Update block averages
    ...
    Mdin.Averages(istep+1);   //Print results for current block
...
void MolDyn::Reset(int iblk) //Reset block averages
{
  if(iblk == 1)
  {
...
void MolDyn::Accumulate(void) //Update block averages
{
  for(int i=0; i<n_props; ++i)
  {
...
void MolDyn::Averages(int iblk) //Print results for current block
{    
  double r, gdir;
  ofstream Gofr, Gave;
  const int wd=12;
  ...
  for(int i=igofr; i<igofr+nbins;i++){
    ...
    double stimag=blk_av[i]/blk_norm/(rho*npart*pi_greco*4/3*(pow(r+dr,3)-pow(r,3)));
    glob_av[i]+=stimag;
...
//g(r)
  if(iblk==N_blocks){
    ...
    for(int i=igofr;i<igofr+nbins;i++){
      ...
      cout<< r << "," << glob_av[i]/(double)iblk << "," <<Error(glob_av[i],glob_av2[i],iblk) <<endl;
